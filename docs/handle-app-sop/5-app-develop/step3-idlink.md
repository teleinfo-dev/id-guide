...
### 后续步骤
1. 依据已创建的产品元数据模板进行[元数据模板授权](#元数据模板授权)
...
### 后续步骤
仓储系统可查看[授权通知信息](#元数据模板授权通知)，并完成对产品[元数据模板的关联](#元数据模板关联)。
...
### 后续步骤
前往同类数据授权（跳转至5.4.3.6）进行授权

## 同类数据授权

### 前置条件
1. 产品元数据模板创建（跳转至5.4.3.2）完成
2. 导入B企业标识身份组（跳转至5.4.2.1.2.1）

### 操作步骤
1. 前往标识注册页面，找到对应元数据模板，根据元数据模板字段层级，对该元数据下所注册的一类标识数据进行批量授权
    <center><img src="./images/idlink/link-12.png" style="margin-top: 20px"/></center>

2. 如图所示，将“箱码”、“产品”、“生产时间”字段设定为查看公开权限，将“规格”、“原料”、“关联入库单”查看权限授权给指定应用
...
3. 使用生产系统所属应用管理员账号登录系统，进行解析，数据如下,可查看全部数据
    <center><img src="./images/idlink/link-15.png" style="margin-top: 20px"/></center>

4. 使用其他账号登录系统，进行解析，数据如下，仅可查看到“箱码”、“产品”、“生产时间”字段信息
...
<p style="font-size: 12px; color: rgb(22,93,255);">说明：产品标识标识数据编码规则为产品元数据标识+选择主键，后续可通过该标识进行解析</p>
...
### 前置条件
B企业完成[入库单元数据模板创建](#元数据模板创建)，并[关联](#元数据模板关联)A企业产品元数据。
...
<p style="font-size: 12px; color: rgb(22,93,255);">
说明：入库单标识标识数据编码规则为产品元数据标识+选择主键，后续可通过该标识进行解析。</p>

### 后续步骤
可根据标识编码规则，进行解析，查看标识数据关系图。
